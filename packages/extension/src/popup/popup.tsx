...
const Popup = () => {
  ...
  return (
    <div className="w-80 p-4 bg-[#faf9f6] text-charcoal">
      <h1 className="text-sm text-center font-bold opacity-50 uppercase tracking-widest">
        Global Horizon Standard
      </h1>

      <div className="my-4 text-center">
        <div className="text-4xl font-mono font-bold text-yellow-600">{now.beatsLong}</div>
        <div className="text-lg opacity-80">{formatGHS(now, 'text')}</div>
      </div>

      <div className="text-xs border-t pt-2 opacity-60 flex justify-between items-end">
        <div>
          <div>{now.weekday}</div>
          <div>{now.month ? `Month ${now.month} (${now.monthName})` : now.monthName}</div>
        </div>
        {now.isAuroraYear && (
